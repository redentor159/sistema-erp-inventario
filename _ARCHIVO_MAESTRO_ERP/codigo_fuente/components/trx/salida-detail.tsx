...
export function SalidaDetail({ id, open, onOpenChange, headerInfo }: SalidaDetailProps) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[80vw] max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        ID: {headerInfo?.id_salida?.slice(0, 8)}... | Tipo: {headerInfo?.tipo_salida}
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4 text-sm">
                    <div>
                        ...
                        <div className="font-medium">{headerInfo?.fecha ? format(new Date(headerInfo.fecha), 'dd/MM/yyyy HH:mm') : '-'}</div>
                    </div>
                    <div>
                        <span className="font-semibold text-muted-foreground">Cliente/Destino:</span>
                        <div className="font-medium">{headerInfo?.mst_clientes?.nombre_completo || headerInfo?.id_destinatario || '-'}</div>
                    </div>
                    <div>
                        <span className="font-semibold text-muted-foreground">Responsable:</span>
                        <div className="font-medium">{headerInfo?.responsable_interno || '-'}</div>
                    </div>
                    <div>
                        <span className="font-semibold text-muted-foreground">Total:</span>
                        <div className="font-bold text-lg">S/ {Number(headerInfo?.total_dinero || 0).toFixed(2)}</div>
                    </div>
                            ...
                            {isLoading ? (
                                ...
                                detalles?.map((item: any) => (
                                    <TableRow key={item.id_linea_salida}>
                                        ...
                                        <TableCell className="text-right">S/ {Number(item.precio_unitario).toFixed(2)}</TableCell>
                                        <TableCell className="text-right font-medium">S/ {Number(item.subtotal).toFixed(2)}</TableCell>
                                    </TableRow>
                                ))
                            )}
                        </TableBody>
