...
export function RetazoForm({ open, onOpenChange }: RetazoFormProps) {
    ...
    const profiles = products?.filter((p: any) =>
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                        ...
                                        <SelectContent className="max-h-[350px] w-[550px]">
                                            ...
                                            <div className="pt-1">
                                                {profiles
                                                    .filter((p: any) =>
                                                        p.id_sku.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                                        p.nombre_completo.toLowerCase().includes(searchTerm.toLowerCase())
                                                    )
                                                    .slice(0, 50)
                                                    .map((p: any) => (
                                                        <SelectItem key={p.id_sku} value={p.id_sku} className="py-3">
                                                            <div className="flex items-center gap-3 w-full">
                                                                <span className="font-mono font-bold text-primary bg-primary/10 px-2 py-0.5 rounded text-sm min-w-[150px]">{p.id_sku}</span>
                                                                <span className="text-sm text-muted-foreground truncate flex-1">{p.nombre_completo}</span>
                                                            </div>
                                                        </SelectItem>
                                                    ))}
