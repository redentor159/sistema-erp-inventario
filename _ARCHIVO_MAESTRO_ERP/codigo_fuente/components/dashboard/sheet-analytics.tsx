...
export function SheetAnalytics() {
    ...
    const [abcType, setAbcType] = useState<'CONSUMPTION' | 'INVENTORY'>('CONSUMPTION')
    ...
    const loadingABC = abcType === 'CONSUMPTION' ? loadingConsumption : loadingInventory
    ...
    const chartData = rawData?.map((item: any) => ({
        ...
        valor: Number(abcType === 'CONSUMPTION' ? item.valor_salida : item.metric_value), // Field name differs
                        ...
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <CardTitle className="text-red-700 dark:text-red-400 flex items-center gap-2">
                                    ...
                                    Monitor de Quiebres
                                    <SimpleTooltip text="Lista en tiempo real de materiales que necesitan reposición urgente (Stock <= Mínimo)." className="text-red-600" />
                                </CardTitle>
                        ...
                        <div className="max-h-[350px] overflow-auto">
                            <Table>
                                ...
                                <TableBody>
                                    ...
                                    {stockRisk?.length === 0 && (
                                        <TableRow>
                                            <TableCell colSpan={3} className="h-24 text-center text-green-600">
                    ...
                    <CardHeader className="pb-2">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-orange-700 dark:text-orange-400 flex items-center gap-2">
                                ...
                                Inventario Zombie
                                <SimpleTooltip text="Productos con Stock > 0 que NO han tenido salidas en los últimos 90 días. Capital estancado que deberías liquidar." className="text-orange-600" />
                            </CardTitle>
                    ...
                    <CardContent className="flex-1 overflow-auto max-h-[350px]">
                        <div className="space-y-4">
                            {zombies?.map((item: any) => (
                                <div key={item.id_sku} className="flex items-center justify-between border-b pb-2 last:border-0 last:pb-0">
                                    <div className="space-y-1">
                                        <p className="text-sm font-medium leading-none">{item.nombre_completo}</p>
                                        <p className="text-xs text-muted-foreground">{item.id_sku} • {item.stock_actual} {item.unidad_medida}</p>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-sm font-bold text-orange-600 block">S/ {item.valor_estancado?.toLocaleString()}</span>
                                        <span className="text-[10px] text-muted-foreground">Inmóvil &gt; 90 días</span>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <CardDescription>
                                        {abcType === 'CONSUMPTION' ? "Valor de Consumo (Movimiento)" : "Valor de Inventario Actual (Estancado)"}
                                    </CardDescription>
                                </div>
                        ...
                        {loadingABC ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <ComposedChart data={chartData} margin={{ top: 20, right: 20, bottom: 60, left: 20 }}>
                                    ...
                                    <XAxis dataKey="name" scale="band" angle={-45} textAnchor="end" interval={0} height={80} fontSize={10} />
                                    ...
                                    <Legend />
                                    <Bar yAxisId="left" dataKey="valor" name={abcType === 'CONSUMPTION' ? "Consumo (S/)" : "Valor Stock (S/)"} barSize={20} fill="#4f46e5">
                                        {chartData?.map((entry: any, index: number) => (
                                            <Cell key={`cell-${index}`} fill={entry.class === 'A' ? "#2563eb" : entry.class === 'B' ? "#60a5fa" : "#94a3b8"} />
                                        ))}
                                    </Bar>
                                    <Line yAxisId="right" type="monotone" dataKey="acumulado" name="% Acumulado" stroke="#ff7300" strokeWidth={2} dot={false} />
                                </ComposedChart>
                            </ResponsiveContainer>
                        )}
                    </CardContent>
                ...
                <Card className="col-span-1 md:col-span-2 lg:col-span-3 border-emerald-200 bg-emerald-50/20 dark:bg-emerald-950/10 h-[450px]">
                    <CardHeader>
                        <CardTitle className="flex items-center text-emerald-700 dark:text-emerald-400 gap-2">
                            ...
                            <SimpleTooltip text="Valor económico estimado de los retazos disponibles." className="text-emerald-600" />
                        </CardTitle>
                    ...
                    <CardContent className="flex flex-col items-center justify-center h-full pb-10 text-center space-y-4">
                        ...
                        <div>
                            <div className="text-4xl font-bold text-emerald-700 dark:text-emerald-400">
                                S/ {retazos?.valor_recuperable_pen?.toLocaleString('es-PE', { maximumFractionDigits: 0 }) ?? 0}
                            </div>
                            <p className="text-sm text-muted-foreground mt-1">Valor Estimado Recuperable</p>
                        </div>
