...
type TokenType = 'NUMBER' | 'VARIABLE' | 'OPERATOR' | 'LPAREN' | 'RPAREN' | 'FUNCTION' | 'COMMA'
    ...
    while (i < s.length) {
        ...
        if (/[a-zA-Z_]/.test(ch)) {
            ...
            if (FUNCTIONS.has(id)) {
                ...
                throw new Error(`Variable desconocida: "${id}". Variables válidas: ${[...VARIABLES].join(', ')}`)
            }
            continue
        }
...
class Parser {
    ...
    private consume(expectedType?: TokenType): Token {
        ...
        if (expectedType && token.type !== expectedType) {
            throw new Error(`Se esperaba ${expectedType}, pero se encontró ${token.type} ("${token.value}")`)
        }
        this.pos++
        return token
    }
    ...
    private parsePrimary(): number {
        ...
        if (token.type === 'VARIABLE') {
            ...
            if (val === undefined) throw new Error(`Variable "${token.value}" no definida`)
            return val
        }
    ...
    private callFunction(name: string, args: number[]): number {
        switch (name) {
            ...
            case 'min':
                if (args.length < 2) throw new Error('min() requiere al menos 2 argumentos')
                return Math.min(...args)
            case 'max':
                if (args.length < 2) throw new Error('max() requiere al menos 2 argumentos')
...
export function previewFormula(
    ...
    const result = evaluateFormula(expr, { ancho, alto, hojas, crucesH: 0, crucesV: 0 })
...
export const FORMULA_FUNCTIONS = [
    ...
    { name: 'round(x)', desc: 'Redondea al entero más cercano', example: 'round(3.5) → 4' },
    { name: 'abs(x)', desc: 'Valor absoluto', example: 'abs(-5) → 5' },
    { name: 'min(a,b)', desc: 'Mínimo entre dos valores', example: 'min(3, 5) → 3' },
    { name: 'max(a,b)', desc: 'Máximo entre dos valores', example: 'max(3, 5) → 5' },
] as const
