...
BEGIN
    ...
    IF v_linea IS NULL THEN
        RAISE NOTICE 'Linea no encontrada';
        RETURN;
    END IF;

    -- Limpiar despiece anterior
    DELETE FROM trx_desglose_materiales WHERE id_linea_cot = p_id_linea_cot AND tipo_componente != 'Servicio';

    v_ancho := v_linea.ancho_mm;
    v_alto := v_linea.alto_mm;
    ...
    LOOP
        ...
        IF v_receta.grupo_opcion IS NOT NULL AND v_linea.opciones_seleccionadas IS NOT NULL THEN
             IF v_linea.opciones_seleccionadas ? v_receta.grupo_opcion THEN
                 v_sku_calculado := v_linea.opciones_seleccionadas ->> v_receta.grupo_opcion;
                 
                 -- Lookup Real Name
                 SELECT nombre_completo INTO v_nombre_real 
        ...
        IF v_sku_calculado IS NULL OR v_sku_calculado = '' THEN
             -- Only calculate if it wasn't a mandatory option that was missing
             -- If it was a generic item that needs calculation:
             IF v_receta.id_sku_catalogo IS NOT NULL OR v_receta.formula_cantidad IS NOT NULL THEN
                v_sku_calculado := fn_calcular_sku_real(
                    ...
                    v_linea.tipo_vidrio,
                    v_linea.id_sistema, -- Pass system context if needed
        ...
        IF v_cantidad_calculada > 0 AND v_sku_calculado IS NOT NULL THEN
            ...
            INSERT INTO trx_desglose_materiales (
                ...
                p_id_linea_cot,
                v_receta.tipo,
                v_nombre_final, -- Uses Real Name if found
