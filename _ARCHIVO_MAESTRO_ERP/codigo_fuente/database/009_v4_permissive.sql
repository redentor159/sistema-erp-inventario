...
BEGIN
    ...
    LOOP
        ...
        IF v_sku_calculado IS NULL OR v_sku_calculado = '' THEN
             IF v_receta.id_sku_catalogo IS NOT NULL OR v_receta.formula_cantidad IS NOT NULL THEN
                ...
                BEGIN
                    ...
                    -- Si fn_calcular_sku_real falla o retorna NULL por falta de material, v_sku_calculado es NULL
        ...
        EXCEPTION WHEN OTHERS THEN
            v_cantidad_calculada := 0; -- Si falla la formula, cantidad 0
        ...
        -- 6. Insert Logic (PERMISSIVE - Insert even with missing SKU/Cost to show in breakdown)
        IF v_cantidad_calculada > 0 THEN
            
            v_costo_total_item := v_cantidad_calculada * v_costo_unit_sku;

            -- If Missing SKU, append warning to name
            IF v_sku_calculado IS NULL OR v_sku_calculado = '' THEN
                v_nombre_final := v_receta.nombre_componente || ' (Falta SKU/Material)';
            END IF;
