...
async function verify() {
    ...
    // Parameters to simulate `catApi.getProductos({ pageSize: 10000 })`
    ...
    while (currentFrom <= endTarget) {
        process.stdout.write(`Fetching range ${currentFrom} - ${Math.min(currentFrom + CHUNK_SIZE - 1, endTarget)}... `)
        ...
        if (data) {
            ...
            console.log(`Got ${data.length} items. Total so far: ${allData.length}`)
        }
