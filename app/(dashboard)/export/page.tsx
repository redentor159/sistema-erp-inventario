...
export default function ExportPage() {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      const startQuery = useDateFilter
        ...
        <CardContent className="pt-5">
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              >
                Filtrar por Fecha de Emisión/Movimiento
              </Label>
            </div>
            ...
            {!useDateFilter && (
              <div className="text-sm text-amber-600 bg-amber-50 px-3 py-2 rounded-md border border-amber-200">
                ⚠️ <strong>Atención:</strong> Descargar todo el historial
                ignorando las fechas puede demorar varios minutos o consumir
                mucha memoria.
              </div>
            )}
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="border-blue-200 hover:border-blue-300 transition-colors shadow-sm relative overflow-hidden group">
          ...
          <CardHeader>
            ...
            <CardDescription>
              Cotizaciones, Detalles, Desglose y Producción.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-slate-600 min-h-[60px]">
              Extrae un archivo con 4 hojas planas. Contiene toda la cabecera
              económica calculada, el detalle granular de ítems, y los cálculos
              de ingeniería generados. Ideal Análisis de Márgenes.
            </p>
          </CardContent>
          <CardFooter>
            <Button
        ...
        <Card className="border-emerald-200 hover:border-emerald-300 transition-colors shadow-sm relative overflow-hidden group">
          ...
          <CardHeader>
            ...
            <CardDescription>
              Stock, Retazos e Inmovilizados (Zombies)
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-slate-600 min-h-[60px]">
              Fotografía en tiempo real del almacén. No requiere filtro de fecha
              (es a "hoy"). Incluye la valorización contable promedio (PMP),
              prioridades de compra y perfiles muertos.
            </p>
          </CardContent>
          <CardFooter>
            <Button
        ...
        <Card className="border-orange-200 hover:border-orange-300 transition-colors shadow-sm relative overflow-hidden group">
          ...
          <CardContent className="space-y-3">
            <p className="text-sm text-slate-600 min-h-[60px]">
              Sábana transaccional que documenta paso a paso qué subió y qué
              bajó en el almacén en el rango de fechas seleccionado. Útil para
              rastrear mermas o descuadres de stock.
            </p>
          </CardContent>
          <CardFooter>
            <Button
        ...
        <Card className="border-purple-200 hover:border-purple-300 transition-colors shadow-sm relative overflow-hidden group">
          ...
          <CardHeader>
            ...
            <CardDescription>
              Dimensiones de Referencia (Clientes, Prov, Catálogo)
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-slate-600 min-h-[60px]">
              Exporta el diccionario de datos (Tablas Dimensionales). Ideal para
              subir a Power BI como tablas Maestras "Catalogos" y cruzar el ID
              con las tablas de Ventas o Movimientos. No usa filtro de fechas.
            </p>
          </CardContent>
          <CardFooter>
            <Button
            ...
            >
              {loading === "master_data" ? (
                ...
                <>
                  <FileSpreadsheet className="mr-2 h-4 w-4" /> Descargar Tablas
                  Maestras
                </>
              )}
            </Button>
