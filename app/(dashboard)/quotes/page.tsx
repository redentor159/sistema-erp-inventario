                ...
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Configure el proyecto y detale los elementos (ventanas, puertas) a cotizar.
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        {quotes?.map((q: any) => (
                            <TableRow key={q.id_cotizacion}>
                                <TableCell className="whitespace-nowrap">
                                    {format(new Date(q.fecha_emision), "dd/MM/yyyy", { locale: es })}
                                </TableCell>
                                <TableCell className="font-mono text-xs">
                                    {/* UUID suffix or real number if implemented */}
                                    ...{q.id_cotizacion.slice(-4)}
                                </TableCell>
                                <TableCell className="font-medium">{q.nombre_proyecto}</TableCell>
                                <TableCell>{q.mst_clientes?.nombre_completo}</TableCell>
                                <TableCell className="font-bold">
                                    {q.total_precio_venta?.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
                                </TableCell>
                                <TableCell>{q.moneda}</TableCell>
                                <TableCell>
                                    <Badge variant={(q.estado === 'APROBADO') ? 'default' : 'secondary'}>
                                        {q.estado}
                                    </Badge>
                                </TableCell>
                            </TableRow>
