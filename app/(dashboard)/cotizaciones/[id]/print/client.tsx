...
export default function CotizacionPrintPage({
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setTexts({
          terms:
            cotRes.terminos_personalizados ||
            confRes.texto_condiciones_base ||
            "",
  ...
  const processText = (text: string) => {
    ...
    const replacements: Record<string, string> = {
      ...
      "{{VALIDEZ}}":
        cotizacion.validez_dias?.toString() ||
        globalConfig.validez_cotizacion_dias?.toString() ||
        "15",
      "{{TOTAL}}": formatCurrency(
        cotizacion._vc_precio_final_cliente,
        cotizacion.moneda as any,
      ),
  ...
  const exchangeRate = globalConfig?.tipo_cambio_referencial || 3.8;
  ...
  const fontThemeClass =
    config.theme === "classic" ? "font-serif" : "font-sans";
  ...
  const handlePrint = () => {
    ...
    // 4. Restaurar el título original casi inmediatamente
          ...
          <div className="px-4 pt-2">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="visual" className="text-xs">
                <Layout className="w-3 h-3 mr-1" /> Visual
              </TabsTrigger>
              <TabsTrigger value="texts" className="text-xs">
                <Type className="w-3 h-3 mr-1" /> Textos
              </TabsTrigger>
              ...
              <div className="space-y-4 border p-4 rounded-lg bg-slate-50/50">
                ...
                <div className="flex items-center justify-between">
                  <Label htmlFor="logo" className="cursor-pointer">
                    Mostrar Logo / Membrete
                  </Label>
                  <Switch
                    id="logo"
                    checked={config.showLogo}
                    onCheckedChange={(c) =>
                      setConfig({ ...config, showLogo: c })
                    }
                  />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(c) =>
                      setConfig({ ...config, showBankInfo: c })
                    }
                  />
                </div>
                <div className="flex items-center justify-between">
                  <Label htmlFor="sign" className="cursor-pointer">
                    Mostrar Espacio Firma
                  </Label>
                  <Switch
                    id="sign"
                    checked={config.showSignature}
                    onCheckedChange={(c) =>
                      setConfig({ ...config, showSignature: c })
                    }
                  />
                </div>
                <div className="flex items-center justify-between">
                  <Label htmlFor="warr" className="cursor-pointer">
                    Mostrar Garantía
                  </Label>
                  <Switch
                    id="warr"
                    checked={config.showWarranty}
                    onCheckedChange={(c) =>
                      setConfig({ ...config, showWarranty: c })
                    }
                  />
                </div>
                <div className="flex items-center justify-between">
            ...
            <TabsContent value="texts" className="space-y-6 mt-0">
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setTexts({
                        ...texts,
                        terms: globalConfig?.texto_condiciones_base || "",
                      })
                    }
                    title="Reset"
                  >
                ...
                <Textarea
                  ...
                  value={texts.terms}
                  onChange={(e) =>
                    setTexts({ ...texts, terms: e.target.value })
                  }
                />
                <div className="flex flex-wrap gap-1">
                  {["{{CLIENTE}}", "{{VALIDEZ}}", "{{TOTAL}}"].map((v) => (
                    <Button
                      ...
                      onClick={() => insertVariable(v, "terms")}
                    >
                      {v}
                    </Button>
                  ))}
              ...
              <div className="space-y-4 pt-4 border-t">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={texts.payment}
                    onChange={(e) =>
                      setTexts({ ...texts, payment: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    className="text-xs h-[50px]"
                    value={texts.warranty}
                    onChange={(e) =>
                      setTexts({ ...texts, warranty: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={texts.notes}
                    onChange={(e) =>
                      setTexts({ ...texts, notes: e.target.value })
                    }
                  />
                </div>
              </div>
        ...
        >
          ...
          <header
            className={cn(
              ...
              isClassic
                ? "text-center"
                : "flex justify-between items-start border-b-2 pb-4",
              isMinimalist && "border-b-0 pb-0",
            )}
            style={{
              borderColor:
                isModern || isMinimalist ? "var(--primary)" : "transparent",
            }}
          >
            ...
            {isClassic && (
              <div className="flex flex-col items-center mb-6">
                ...
                <div className="text-sm font-serif text-slate-600 space-y-1">
                  <p className="font-bold tracking-wide uppercase text-slate-800">
                  ...
                  <p className="text-xs italic">
                    {globalConfig?.direccion_fiscal}
                  </p>
                  <p className="text-xs text-blue-800 underline">
                    {globalConfig?.email_empresa}
                  </p>
                </div>
                ...
                <div className="w-full flex justify-between items-end px-4">
                  <div className="text-left">
                    <h2 className="text-2xl font-serif font-bold uppercase tracking-widest text-slate-900">
                      {config.title}
                    </h2>
                  </div>
              ...
              <>
                <div className="w-[60%]">
                  {config.showLogo && (
                    <div>
                      {globalConfig?.logo_url ? (
                        ...
                        <h1
                          className="text-2xl font-bold text-slate-900 uppercase tracking-tighter"
                  ...
                  {config.showLogo && globalConfig?.logo_url && (
                    <h2
                      className={cn("text-lg font-bold uppercase")}
                      style={{ color: "var(--primary)" }}
                    >
                      {globalConfig?.nombre_empresa}
                    </h2>
                  ...
                  <div
                    className={cn(
                      "space-y-0.5 mt-2 text-sm text-slate-600",
                      isMinimalist && "text-xs font-light text-slate-400",
                    )}
                  >
                    <p>
                    ...
                    <p>
                      {globalConfig?.telefono_contacto} |{" "}
                      {globalConfig?.email_empresa}
                    </p>
                  </div>
                </div>
                ...
                >
                  <h2
                    className={cn(
                      "text-3xl font-bold uppercase mb-1",
                      isMinimalist && "text-2xl font-light tracking-wide",
                    )}
                    style={{ color: "var(--primary)" }}
                  >
                    {config.title}
                  ...
                  <div className="mt-3 text-sm space-y-1">
                    <p>
                      <strong>Fecha:</strong> {processText("{{FECHA}}")}
                    </p>
                    <p>
                    ...
                    <p>
                      <strong>Validez:</strong> {processText("{{VALIDEZ}}")}{" "}
                      días
                    </p>
                    <p>
            ...
            <div className="grid grid-cols-12 gap-4 text-sm">
              <div className="col-span-8">
                ...
                <p className={cn("text-slate-600", isClassic && "italic")}>
                  {cotizacion.mst_clientes?.direccion_obra_principal ||
              ...
              <tbody className="text-slate-700">
                ...
                {(cotizacion.costo_fijo_instalacion ?? 0) > 0 && (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <div className="font-bold text-slate-900">
                        Servicios de Instalación y Logística
                      </div>
                      <div className="text-xs text-slate-500 mt-1 italic">
                        Incluye: Embalaje, Flete, Movilidad, Viáticos, SCTR e
                        Instalación en obra.
                      </div>
                    </td>
                    ...
                    <td className="p-3 text-right text-slate-600 font-mono">
                      {formatCurrency(
                        cotizacion.costo_fijo_instalacion!,
                        quoteCurrency as any,
                      )}
                    </td>
                    <td className="p-3 text-right font-bold text-slate-800 font-mono">
            ...
            >
              <div className="space-y-2 mb-3">
                <div className="flex justify-between text-sm text-slate-600">
                  <span className={isClassic ? "font-serif" : ""}>
                    Subtotal ({quoteCurrency}):
                  </span>
                  <span>
                    {formatCurrency(totalQuote / 1.18, quoteCurrency as any)}
                  </span>
                </div>
                <div className="flex justify-between text-sm text-slate-600">
                  <span className={isClassic ? "font-serif" : ""}>
                ...
                >
                  <span
                    className={
                      isClassic ? "uppercase tracking-widest font-serif" : ""
                    }
                  >
                    Total {quoteCurrency}:
                  </span>
                  <span
                    style={{ color: isClassic ? "black" : "var(--primary)" }}
                  >
                    {formatCurrency(totalQuote, quoteCurrency as any)}
                  </span>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-dashed border-slate-300 text-xs text-slate-500 text-right">
                ...
                <div className="flex justify-between items-center mt-1 font-semibold text-slate-700">
                  ...
                  <span className="text-sm">
                    {formatCurrency(totalMain, mainCurrency as any)}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-3 pt-2 border-t border-slate-200 text-[10px] text-slate-500 italic text-right">
                {numeroALetras(
                  cotizacion.incluye_igv ? totalQuote : totalQuote,
                  quoteCurrency as any,
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <h4
                  className={cn(
                    ...
                    isClassic
                      ? "border-b-2 border-black text-black font-serif"
                      : "border-b",
                  )}
                  style={{
              ...
              {texts.notes && (
                <div className="pt-2 border-t border-slate-200 mt-2">
                  <h4 className="font-bold uppercase text-[10px] text-slate-500 mb-1">
                    Observaciones
                  </h4>
            ...
            <div className="flex flex-col justify-between h-full">
              ...
              {config.showBankInfo && (
                ...
                >
                  <h4
                    className={cn(
                      "font-bold mb-2 uppercase text-center",
                      isClassic && "text-black font-serif tracking-widest",
                    )}
                    style={{ color: isClassic ? "black" : "var(--primary)" }}
                  >
                    Cuentas Bancarias
                  ...
                  <div className={cn("space-y-2", isClassic && "font-serif")}>
                    ...
                    {(globalConfig?.cuenta_bcp_dolares ||
                      ...
                      (quoteCurrency === "USD" || mainCurrency === "USD") && (
                        <div className="mt-2 pt-2 border-t border-dashed border-slate-300">
                          <p className="font-bold">BCP Dólares ($)</p>
                          {globalConfig.cuenta_bcp_dolares && (
                            <p>Cta: {globalConfig.cuenta_bcp_dolares}</p>
                          )}
                          {globalConfig.cci_dolares && (
                            <p>CCI: {globalConfig.cci_dolares}</p>
                          )}
                        </div>
                      )}
                    ...
                    {(globalConfig?.cuenta_bbva_dolares ||
                      ...
                      (quoteCurrency === "USD" || mainCurrency === "USD") && (
                        <div className="mt-2 pt-2 border-t border-dashed border-slate-300">
                          ...
                          {globalConfig.cuenta_bbva_dolares && (
                            <p>Cta: {globalConfig.cuenta_bbva_dolares}</p>
                          )}
                          {globalConfig.cci_bbva_dolares && (
                            <p>CCI: {globalConfig.cci_bbva_dolares}</p>
                          )}
                        </div>
                      )}
                  </div>
                ...
                <div className="flex flex-col items-center mt-auto pt-8">
                  <div className="w-[180px] h-[80px] mb-1 relative flex items-center justify-center">
                    {globalConfig?.firma_digital_url && (
                      <img
                        src={globalConfig.firma_digital_url}
                        alt="Firma"
                  ...
                  <p className="font-bold text-slate-800 text-xs">
                    {globalConfig?.nombre_representante ||
                      globalConfig?.nombre_empresa}
                  </p>
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest">
                    {globalConfig?.cargo_representante || "Gerencia General"}
                  </p>
                </div>
              )}
            </div>
