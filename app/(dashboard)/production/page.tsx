...
export default function ProductionPage() {
  ...
  const [copiedOrder, setCopiedOrder] = useState<Partial<KanbanOrder> | null>(
    null,
  );
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Orden Creada",
        description: "La orden se ha añadido al tablero.",
      });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<KanbanOrder> }) =>
      kanbanApi.updateOrder(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["kanbanBoard"] });
      toast({
  ...
  const moveMutation = useMutation({
    mutationFn: ({ id, col, idx }: { id: string; col: string; idx: number }) =>
      kanbanApi.moveCard(id, col, idx),
    onMutate: async ({ id, col, idx }) => {
      ...
      const previousOrders = queryClient.getQueryData<KanbanOrder[]>([
        "kanbanBoard",
      ]);
  ...
  const legacyHistory = (historyData?.data || []).map((h) =>
    adaptHistoryToLegacy(h as any),
  );
  ...
  const handleCopyOrder = (order: KanbanOrder) => {
    ...
    toast({
      title: "Copiado",
      description: "Puedes pegar esta orden como una nueva.",
    });
  };
  ...
  const archiveAllMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Las órdenes finalizadas han sido archivadas.",
      });
    },
  });
  ...
  const handleArchiveAll = () => {
    if (
      confirm(
        "¿Estás seguro de archivar TODAS las órdenes finalizadas? Se moverán al historial.",
      )
    ) {
