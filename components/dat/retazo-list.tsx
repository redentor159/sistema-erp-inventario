...
export function RetazosList() {
  ...
  const paginatedRetazos = retazos?.slice(
    page * pageSize,
    (page + 1) * pageSize,
  );
    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row justify-between gap-4 p-4 border rounded-lg bg-gray-50 dark:bg-gray-900/50 items-center">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <Button onClick={() => setIsFormOpen(true)}>+ Nuevo Retazo</Button>
        </div>
      </div>
      ...
      <div className="border rounded-md bg-white dark:bg-gray-800 overflow-hidden">
              ...
              {paginatedRetazos?.map((item) => (
                <TableRow key={item.id_retazo}>
                  <TableCell className="text-xs text-muted-foreground">
                    {format(new Date(item.fecha_creacion), "dd/MM/yy", {
                      locale: es,
                    })}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right">
                    ...
                    {item.estado === "USADO" && (
                      <span className="text-xs text-muted-foreground">
                        {item.fecha_consumo
                          ? format(new Date(item.fecha_consumo), "dd/MM", {
                              locale: es,
                            })
                          : "Consumido"}
                      </span>
        ...
        <div className="flex items-center justify-between px-4 py-4 border-t">
          <div className="text-sm text-muted-foreground">
            Mostrando {retazos && retazos.length > 0 ? page * pageSize + 1 : 0}{" "}
            a {Math.min((page + 1) * pageSize, retazos?.length || 0)} de{" "}
            {retazos?.length || 0}
          </div>
