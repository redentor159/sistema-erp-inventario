  ...
  if (loadingVal || loadingOtif) {
    return (
      <div className="p-10 text-center animate-pulse">
        Cargando indicadores estratégicos...
      </div>
    );
  }
      ...
      <div className="flex justify-between items-center bg-muted/30 p-4 rounded-lg border">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Estado financiero del inventario y nivel de servicio.
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <Label htmlFor="currency-mode" className="font-bold">
            PEN
          </Label>
          <Switch
            id="currency-mode"
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CheckCircle2
              className={`h-4 w-4 ${otifPct >= 95 ? "text-green-500" : "text-yellow-500"}`}
            />
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-4">
          <CardHeader>
            ...
            <CardDescription>
              Evolución del nivel de servicio últimos 6 meses
            </CardDescription>
          </CardHeader>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={otifData ? [...otifData].reverse() : []}>
                  ...
                  <Tooltip
                    ...
                    labelFormatter={
                      ((label: string) =>
                        new Date(label).toLocaleDateString()) as any
                    }
                  />
                  <Bar dataKey="pct_otif" radius={[4, 4, 0, 0]}>
                    {otifData?.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.pct_otif >= 90 ? "#22c55e" : "#eab308"}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        <Card className="col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              Ventas vs Costos Directos (Mes Actual)
            </CardDescription>
          </CardHeader>
          <CardContent className="flex items-center justify-center h-[300px]">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-xs">
                Registre salidas de venta para visualizar.
              </p>
            </div>
          </CardContent>
        </Card>
