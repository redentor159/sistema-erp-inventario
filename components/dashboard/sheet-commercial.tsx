...
export function SheetCommercial() {
  ...
  const { data: ticket } = useQuery({
    queryKey: ["dashTicket"],
    queryFn: dashboardApi.getTicket,
  });
  ...
  const { data: margin } = useQuery({
    queryKey: ["dashMargen"],
    queryFn: dashboardApi.getMargin,
  });
  const { data: cycle } = useQuery({
    queryKey: ["dashCycle"],
    queryFn: dashboardApi.getSalesCycle,
  });
      ...
      <div className="flex justify-between items-center bg-muted/30 p-4 rounded-lg border">
        <div>
          <h3 className="text-lg font-medium">
            Inteligencia Comercial & Financiera
          </h3>
          <p className="text-sm text-muted-foreground">
            Métricas reales de ventas, márgenes y eficiencia de cierre.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {conversion?.tasa_conversion_pct ?? 0}%
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              S/{" "}
              {ticket?.ticket_promedio_pen?.toLocaleString("es-PE", {
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {cycle?.dias_promedio_cierre ?? 0} días
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="col-span-4">
          <CardHeader>
            ...
            <CardDescription>
              Volumen de ventas por Modelo/Sistema
            </CardDescription>
          </CardHeader>
