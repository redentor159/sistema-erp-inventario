...
import { Loader2, Plus, Trash2, Save, AlertTriangle, Wrench } from "lucide-react"
...
export function CotizacionIngenieriaManualDialog({ idLinea, isOpen, onOpenChange }: Props) {
    const queryClient = useQueryClient()
    const { toast } = useToast()
    const [localItems, setLocalItems] = useState<Partial<TrxDesgloseMateriales>[]>([])
    ...
    const updateMutation = useMutation({
        mutationFn: async (items: Partial<TrxDesgloseMateriales>[]) => {
            ...
            const currentIds = new Set(items.map(i => i.id_desglose).filter(Boolean))

            // Deletes
            const toDelete = serverItems?.filter(i => !currentIds.has(i.id_desglose)) || []
        ...
        onError: (err) => {
            toast({ title: "Error al guardar ingeniería", description: err.message, variant: "destructive" })
        }
    })
    ...
    const updateRow = (index: number, field: keyof TrxDesgloseMateriales, value: any) => {
        const newItems = [...localItems]
            ...
            <DialogContent className="max-w-[95vw] w-full h-[95vh] flex flex-col p-0 gap-0 sm:max-w-[95vw]">
                <DialogHeader className="px-6 py-4 border-b">
                    ...
                    <DialogDescription className="flex items-center gap-2 text-orange-700 bg-orange-50 p-2 rounded border border-orange-200">
                        ...
                        Advertencia: Al editar manualmente, este ítem dejará de calcularse automáticamente. Si cambias las medidas (Ancho/Alto) después, se te pedirá confirmación para sobreescribir estos cambios.
                    </DialogDescription>
                </DialogHeader>
                                        ...
                                        <TableRow key={idx}>
                                            <TableCell>
                                                <Select
                                                    ...
                                                    onValueChange={(val) => updateRow(idx, 'tipo_componente', val)}
                                                >
                                            ...
                                            <TableCell>
                                                <CatalogSkuSelector
                                                    ...
                                                    onChange={(sku, product) => {
                                                        ...
                                                        if (product) {
                                                            updates.nombre_componente = product.nombre_completo
                                                            updates.costo_total_item = product.costo_mercado_unit || 0

                                                            // Auto-detect type based on parts or prefix
                                                            if (sku.startsWith('AL-')) updates.tipo_componente = 'Perfil'
                                                            else if (sku.startsWith('GEN-')) updates.tipo_componente = 'Accesorio'
                                                            else if (sku.startsWith('VID-')) updates.tipo_componente = 'Vidrio'

                                                            // For profiles, set measure if formula is empty
                                                            if (updates.tipo_componente === 'Perfil' && !item.detalle_formula) {
                                            ...
                                            <TableCell>
                                                <Input
                                                    ...
                                                    onChange={(e) => updateRow(idx, 'nombre_componente', e.target.value)}
                                                    className="h-8"
                                                />
                                            </TableCell>
                                            <TableCell>
                                                <Input
                                                    ...
                                                    onChange={(e) => updateRow(idx, 'detalle_formula', e.target.value)}
                                            ...
                                            <TableCell>
                                                <Input
                                                    ...
                                                    onChange={(e) => updateRow(idx, 'medida_corte_mm', parseFloat(e.target.value))}
                                            ...
                                            <TableCell>
                                                <Input
                                                    ...
                                                    onChange={(e) => updateRow(idx, 'cantidad_calculada', parseFloat(e.target.value))}
                                                    className="h-8 text-right"
                                                    step="0.01"
                                                />
                                            </TableCell>
                                            <TableCell>
                                                <Input
                                                    ...
                                                    onChange={(e) => updateRow(idx, 'costo_total_item', parseFloat(e.target.value))}
                                                    className="h-8 text-right"
                                                    step="0.01"
                                                />
                                            </TableCell>
                                            <TableCell className="text-right font-medium">
                                                {formatCurrency((item.costo_total_item || 0) * (item.cantidad_calculada || 1))}
                ...
                <div className="px-6 py-2 border-t bg-white">
                    <Button variant="outline" size="sm" onClick={addNewRow} className="gap-2">
