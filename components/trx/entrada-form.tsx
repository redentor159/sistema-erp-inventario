...
export function EntradaFormCmp({ onSuccess }: EntradaFormProps) {
  ...
  const subtotal =
    detalles?.reduce((acc, curr) => {
      const lineTotal =
        Number(curr.cantidad) * Number(curr.costo_unitario) -
        (Number(curr.descuento) || 0);
                  ...
                  <TableRow key={field.id}>
                          ...
                          <FormControl>
                            <InlineProductCombobox
                              ...
                              onChange={(val, product) => {
                                ...
                                if (product) {
                                  const currentCost =
                                    product.costo_estandar || 0;
                                  form.setValue(
                                    `detalles.${index}.costo_unitario`,
                                    currentCost,
                                  );
                                }
                              }}
                    ...
                    <TableCell className="text-right font-medium">
                      {(
                        ...
                        (form.watch(`detalles.${index}.costo_unitario`) || 0)
                      ).toFixed(2)}
                    </TableCell>
                    <TableCell>
                      <Button
