...
const DetailRow = ({
  ...
  <div
    className={`grid grid-cols-3 gap-4 py-2 border-b last:border-0 ${className}`}
  >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] p-0 flex flex-col gap-0">
        <DialogHeader className="p-6 pb-2">
          ...
          <DialogDescription>ID: {data.id_movimiento}</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="bg-muted/30 rounded-lg p-3 border">
                ...
                <DetailRow
                  label="Familia / Marca"
                  value={`${data.nombre_familia || "-"} / ${data.nombre_marca || "-"}`}
                />
                <DetailRow
                  label="Cantidad"
                  value={
                    <span
                      className={`font-bold ${data.cantidad < 0 ? "text-red-500" : "text-green-600"}`}
                    >
                ...
                <DetailRow
                  ...
                  value={
                    data.costo_unit_doc
                      ? `S/ ${Number(data.costo_unit_doc).toFixed(4)}`
                      : "-"
                  }
                />
                <DetailRow
                  label="Total PEN"
                  value={
                    data.costo_total_pen
                      ? `S/ ${Number(data.costo_total_pen).toFixed(2)}`
                      : "-"
                  }
                />
              </div>
