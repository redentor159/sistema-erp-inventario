...
export function CotizacionItemDialog({
  idCotizacion,
  onItemAdded,
  triggerButton,
  itemToEdit,
  ...
  const recipesOptionsByModel =
    recetasOptions?.reduce((acc: any, r: any) => {
      if (!acc[r.id_modelo]) acc[r.id_modelo] = {};
      if (!acc[r.id_modelo][r.grupo_opcion])
        acc[r.id_modelo][r.grupo_opcion] = [];
  ...
  useEffect(() => {
    if (open && itemToEdit) {
      if (itemToEdit.id_modelo === "SERVICIO") {
        ...
        const foundModel = allModelos?.find(
          (m: any) => m.id_modelo === itemToEdit.id_modelo,
        );
  ...
  useEffect(() => {
    ...
    const selectedGlass = vidrios.find(
      (v: any) => v.id_sku === item.tipo_vidrio,
    );
    ...
    try {
      if (activeTab === "producto") {
        ...
        if (!servicio.descripcion || servicio.precio_unitario <= 0) {
          toast.warning(
            "Datos incompletos",
            "Ingrese descripción y precio válido",
          );
          setLoading(false);
          return;
        }
        ...
        if (itemToEdit) {
          ...
          toast.warning(
            "Edición de servicios aún no implementada completamente",
            "Por favor elimine y vuelva a crear si necesita cambiar precios.",
          );
        } else {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {itemToEdit ? "Editar Ítem" : "Agregar Ítem a Cotización"}
          </DialogTitle>
          <DialogDescription>
            {itemToEdit
              ? "Modifique los datos del ítem."
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger
              value="producto"
              disabled={!!itemToEdit && itemToEdit.id_modelo === "SERVICIO"}
            >
              Producto (Ventana/Puerta)
            </TabsTrigger>
            <TabsTrigger
              value="servicio"
              disabled={!!itemToEdit && itemToEdit.id_modelo !== "SERVICIO"}
              ...
              <div className="grid gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setItem({ ...item, etiqueta_item: e.target.value })
                      }
                      placeholder="Ej: V-01"
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setItem({ ...item, ubicacion: e.target.value })
                      }
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      value={item.ancho_mm}
                      onChange={(e) =>
                        setItem({ ...item, ancho_mm: Number(e.target.value) })
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      value={item.alto_mm}
                      onChange={(e) =>
                        setItem({ ...item, alto_mm: Number(e.target.value) })
                      }
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      value={item.cantidad}
                      onChange={(e) =>
                        setItem({ ...item, cantidad: Number(e.target.value) })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="SIN_VIDRIO">
                          Sin Vidrio / No Aplica
                        </SelectItem>
                        {vidrios?.map((v: any) => (
                ...
                {item.id_modelo && recipesOptionsByModel[item.id_modelo] && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4">
                    {Object.entries(recipesOptionsByModel[item.id_modelo]).map(
                      ([grupo, opts]) => {
                        const isMandatory =
                          (opts as any[])[0]?.condicion === "BASE";
                        return (
                    ...
                    <div className="mt-4 p-4 border rounded-lg bg-slate-50 border-slate-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="font-semibold text-slate-800 text-sm">
                          Costo Adicional: Embalaje y Flete del cristal
                        </h4>
                      </div>

                      {isTemplado && (
                        <div className="mb-3 p-2 bg-orange-50 border border-orange-200 rounded text-xs text-orange-800">
                          <strong>Flete Recomendado:</strong> El vidrio
                          seleccionado es Templado ({thickness}mm). Se
                          recomienda encarecidamente incluir transporte
                          especializado desde planta externa.
                        </div>
                      )}
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setItem((prev) => {
                                ...
                                newOptions.incluir_flete = checked
                                  ? "true"
                                  : "false";
                                if (checked && !newOptions.factor_flete) {
                                  newOptions.factor_flete =
                                    suggestedFactor.toString();
                                } else if (!checked) {
                          ...
                          >
                            {isTemplado
                              ? "Incluir Embalaje + Flete Recomendado"
                              : "Incluir Embalaje + Flete"}
                          </Label>
                        </div>
                        ...
                        {includeFlete && (
                          <div className="grid gap-2 pl-6">
                            ...
                            <div className="flex items-center gap-2">
                              <Select
                                value={
                                  ["15", "20", "25"].includes(currentFactorStr)
                                    ? currentFactorStr
                                    : currentFactorStr
                                      ? "Otro"
                                      : suggestedFactor.toString()
                              ...
                              <span className="text-xs text-slate-500">
                                Sugerido para{" "}
                                {isTemplado
                                  ? `Templado ${thickness}mm`
                                  : "Crudo/Laminado"}
                                : {suggestedFactor}
                              </span>
                            </div>
                            <p className="text-[10px] text-muted-foreground mt-1">
                              Fórmula: Cantidad = Área (m²) × Factor. Costo =
                    ...
                    <div className="grid gap-2">
                      ...
                      {itemToEdit && (
                        <p className="text-xs text-red-500">
                          Para cambiar precio, elimine y cree de nuevo.
                        </p>
                      )}
                      <p className="text-xs text-muted-foreground">
                        El Markup se aplicará sobre este precio.
                      </p>
                    </div>
                  </div>
                </div>
