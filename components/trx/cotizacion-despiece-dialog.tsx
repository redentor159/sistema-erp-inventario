...
export function CotizacionDespieceDialog({ idCotizacion, idLineaCot, trigger }: CotizacionDespieceDialogProps) {
    ...
    const processedData = (reporte || [])
        ...
        .sort((a, b) => {
            ...
            if (a.tipo_componente === 'Perfil' && b.tipo_componente !== 'Perfil') return -1
            if (a.tipo_componente !== 'Perfil' && b.tipo_componente === 'Perfil') return 1
            return 0
        })
    ...
    // Calculate Totals
    const totalCostoDirecto = data.reduce((sum, item) => sum + (item._displayCost || 0), 0)
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-[98vw] w-full h-[95vh] flex flex-col p-0 gap-0 sm:max-w-[98vw]">
                <DialogHeader className="px-6 py-4 border-b">
                    <DialogTitle>
                        {isGlobal ? 'Desglose Global de Materiales (Proyecto Completo)' : 'Desglose Unitario de Ingeniería (Por Ítem)'}
                    </DialogTitle>
                    <DialogDescription>
                        {isGlobal
                ...
                <div className="flex-1 overflow-hidden p-6">
                    <Tabs defaultValue="detallado" className="w-full h-full flex flex-col">
                        ...
                        <TabsContent value="detallado" className="flex-1 overflow-auto border rounded-md relative flex flex-col">
                                                    ...
                                                    <TableCell className="text-right text-xs">
                                                        {row.tipo_componente === 'Perfil' ? (
                                                            <>
                                                                ...
                                                                <span className="text-muted-foreground ml-1">x {row._displayQty?.toFixed(2)}</span>
                                                            </>
                                                        ) : (
                                                            <span>{row._displayQty?.toFixed(2)} {row.unidad_medida}</span>
                                                        )}
                                                    </TableCell>
                            ...
                            <div className="sticky bottom-0 bg-slate-100 border-t p-4 flex flex-col md:flex-row justify-between items-center z-10 gap-4">
                                ...
                                <div className="flex items-center gap-8 bg-white p-2 rounded-md border shadow-sm">
                                    <div className="flex flex-col items-end">
                                        <span className="text-[10px] uppercase text-slate-500 font-bold">Costo Directo {isGlobal ? 'Total' : 'Unitario'}</span>
                                        <span className="text-lg font-bold text-slate-700">{formatCurrency(totalCostoDirecto)}</span>
                                    </div>

                                    <div className="h-8 w-px bg-slate-300"></div>

                                    <div className="flex flex-col items-end">
                                        <span className="text-[10px] uppercase text-slate-500 font-bold">Markup ({markupPercentage}%)</span>
                                        <span className="text-sm font-mono text-slate-600">x {(1 + margin).toFixed(2)}</span>
                                    </div>

                                    <div className="h-8 w-px bg-slate-300"></div>

                                    <div className="flex flex-col items-end">
                                        <span className="text-[10px] uppercase text-blue-600 font-bold">Precio Venta {isGlobal ? 'Total' : 'Unitario'}</span>
                                        <span className="text-xl font-bold text-blue-700">{formatCurrency(totalPrecioVenta)}</span>
                                    </div>
                                </div>
                            </div>
                        </TabsContent>
