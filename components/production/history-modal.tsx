              ...
              {movements.length > 0 ? (
                <div className="space-y-2">
                  {movements.map((move: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex flex-col bg-muted/40 p-3 rounded-lg text-sm border-l-4 border-l-blue-500"
                    >
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground mt-1">
                        <span>
                          Entrada:{" "}
                          {new Date(move.entryDate).toLocaleDateString()}
                        </span>
                        {move.exitDate && (
                          <span>
                            Salida:{" "}
                            {new Date(move.exitDate).toLocaleDateString()}
                          </span>
                        )}
                      </div>
                    </div>
              ...
              {reworks.length > 0 ? (
                <div className="space-y-2">
                  {reworks.map((rw: any, idx: number) => (
                    <div
                      key={idx}
                      className="bg-orange-50 dark:bg-orange-900/10 p-3 rounded-lg text-sm border-l-4 border-l-orange-500"
                    >
                      <div className="font-medium text-orange-800 dark:text-orange-200">
                        {rw.from} <ArrowRight className="inline h-3 w-3" />{" "}
                        {rw.to}
                      </div>
