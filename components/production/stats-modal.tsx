...
const TopListItem = ({ item, count }: { item: string; count: number }) => (
  <li className="flex justify-between items-center py-2 px-3 border-b border-gray-100 last:border-0 text-sm">
    <span className="font-medium truncate max-w-[200px]" title={item}>
      {item}
    </span>
...
export function StatsModal({
  ...
  const periodData = useMemo(() => {
    ...
    switch (filter) {
      case "7days":
        ...
        previousPeriodEnd = new Date(
          currentPeriodStart.getTime() - 1 * 24 * 60 * 60 * 1000,
        );
        previousPeriodStart = new Date(
          previousPeriodEnd.getTime() - 6 * 24 * 60 * 60 * 1000,
        );
        break;
      case "year":
        currentPeriodEnd = new Date(today);
      ...
      case "30days":
        ...
        previousPeriodStart = new Date(
          previousPeriodEnd.getTime() - 29 * 24 * 60 * 60 * 1000,
        );
        break;
      case "all":
      default:
        ...
        const earliest =
          history.length > 0
            ? Math.min(
                ...history.map((p) => new Date(p.creationDate).getTime()),
              )
    ...
    const getStatsForPeriod = (start: Date, end: Date) => {
      ...
      const completed = periodHistory.filter(
        (p) =>
          (p.status === "Finalizado" || p.status === "Archivado") &&
          p.completionDate &&
          p.creationDate,
      );

      const projectsWithRework = periodHistory.filter(
        (p) => (p.reworkCount || 0) > 0,
      );
      const reworkRate =
        periodHistory.length > 0
          ? (projectsWithRework.length / periodHistory.length) * 100
          : 0;

      const totalDeliveryDays = completed.reduce(
        (sum, p) => sum + calculateDays(p.creationDate, p.completionDate!),
        0,
      );
      const avgDeliveryTime =
        completed.length > 0 ? totalDeliveryDays / completed.length : 0;
    ...
    const previousStats = hasComparison
    ...
    const completedChange = previousStats
      ? calculateChange(
          currentStats.completed.length,
          previousStats.completed.length,
        )
      : 0;
    const deliveryTimeChange = previousStats
      ? calculateChange(
          currentStats.avgDeliveryTime,
          previousStats.avgDeliveryTime,
        )
      : 0;
    const reworkRateChange = previousStats
      ? calculateChange(currentStats.reworkRate, previousStats.reworkRate)
      : 0;

    const reworkPaths: { [key: string]: number } = {};
    const projectsWithReworkInCurrentPeriod = currentStats.history.filter(
      (p) => p.reworkCount && p.reworkCount > 0,
    );
    ...
    const topReworkPaths = Object.entries(reworkPaths)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 5);

    const stageDurations: { [stage: string]: number[] } = {};
    COLUMN_IDS.filter((id) => id !== "column-finalizado").forEach((id) => {
      stageDurations[COLUMN_TITLES[id]] = [];
    });
      ...
      project.movementHistory?.forEach((move: any) => {
        ...
        if (move.exitDate && stageTitle && stageDurations[stageTitle]) {
          ...
          if (duration >= 0 && duration < 90) {
            stageDurations[stageTitle].push(duration);
          }
        }
      });
    });
    const avgStageTimes = Object.entries(stageDurations).map(
      ([stage, durations]) => ({
        stage,
        avgTime: parseFloat(
          (durations.length > 0
    ...
    const leadTimes = currentStats.completed.map((p) =>
      calculateDays(p.creationDate, p.completionDate!),
    );
    const bins = [
      { label: "0-7 d", min: 0, max: 7, count: 0 },
      { label: "8-14 d", min: 8, max: 14, count: 0 },
      { label: "15-21 d", min: 15, max: 21, count: 0 },
      { label: "22-30 d", min: 22, max: 30, count: 0 },
  ...
  const CycleTimeScatterPlot = () => {
    const { data, percentile50, percentile85, domainX, domainY } =
      periodData.scatterPlot;
    if (data.length === 0)
      return (
        <p className="text-gray-500 text-center py-4">
          No hay datos de proyectos finalizados para el gráfico de dispersión.
        </p>
      );

    const width = 800,
      height = 450; // Larger aspect ratio
    ...
    const xScale = (date: Date) =>
      ((date.getTime() - minX.getTime()) / (maxX.getTime() - minX.getTime())) *
      innerWidth;
        ...
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          ...
          <g transform={`translate(0, ${yScale(percentile50)})`}>
            ...
            <text
              ...
              y={4}
              textAnchor="end"
              className="text-xs font-semibold fill-current text-amber-600"
            >
            ...
            <text
              x={innerWidth + 5}
              y="3"
              className="text-xs fill-current text-amber-600 font-semibold"
            >
              p50
            </text>
          </g>
          <g transform={`translate(0, ${yScale(percentile85)})`}>
            ...
            <text
              ...
              y={4}
              textAnchor="end"
              className="text-xs font-semibold fill-current text-red-600"
            >
          ...
          {data.map((d, i) => (
            <circle
              key={`${d.id}-${i}`}
              cx={xScale(d.completed)}
              cy={yScale(d.leadTime)}
              r="3.5"
              className="fill-current text-indigo-500 opacity-70 hover:opacity-100 cursor-pointer"
            >
      ...
      <DialogContent className="max-w-[95vw] !max-w-[95vw] w-[95vw] !w-[95vw] max-h-[96vh] h-[96vh] flex flex-col p-6 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <StatCard
            ...
            comparison={{
              value: periodData.summaryStats.comparison.completedChange,
            }}
          />
          <StatCard
            ...
            comparison={{
              value: periodData.summaryStats.comparison.deliveryTimeChange,
            }}
            invertColors={true}
          />
          <StatCard
            ...
            comparison={{
              value: periodData.summaryStats.comparison.reworkRateChange,
            }}
            invertColors={true}
          />
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white dark:bg-card p-6 rounded-lg border shadow-sm">
              <h3 className="text-lg font-semibold mb-6 text-center text-gray-700 dark:text-gray-300">
                Análisis de Tiempo de Ciclo por Etapa
              </h3>
                  ...
                  >
                    ...
                    <div className="h-8 bg-gray-100 rounded-full overflow-hidden relative w-full">
                      <div
                        ...
                        style={{
                          width: `${Math.max((d.avgTime / periodData.cycleTime.maxTime) * 100, 15)}%`,
                        }} // Min 15% width for text visibility
                      >
            ...
            <div className="bg-white dark:bg-card p-6 rounded-lg border shadow-sm">
              ...
              <div className="h-64 flex items-end justify-between gap-2 px-4">
                {periodData.leadTimeDistribution.bins.map((bin, i) => {
                  const heightPercent =
                    bin.count > 0
                      ? (bin.count / periodData.leadTimeDistribution.maxCount) *
                        100
                      : 0;
                  return (
                    <div
                    ...
                    >
                      <span className="text-xs font-bold text-gray-600 mb-2 group-hover:scale-110 transition-transform">
                        {bin.count}
                      </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-card p-6 rounded-lg border shadow-sm">
              <h3 className="text-sm font-bold uppercase tracking-wider mb-4 text-gray-500">
                Top 5 Productos Más Fabricados
              </h3>
            ...
            <div className="bg-white dark:bg-card p-6 rounded-lg border shadow-sm">
              <h3 className="text-sm font-bold uppercase tracking-wider mb-4 text-gray-500">
                Puntos de Falla (Retrabajos)
              </h3>
              {periodData.reworkAnalysis.topReworkPaths.length > 0 ? (
                <ul className="space-y-1">
                  {periodData.reworkAnalysis.topReworkPaths.map(
                    ([path, count]) => (
