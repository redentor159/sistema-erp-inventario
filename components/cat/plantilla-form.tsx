...
import {
  plantillaSchema,
  PlantillaForm as PlantillaFormType,
...
export function PlantillaForm({ onSuccess, initialData }: PlantillaFormProps) {
  ...
  const form = useForm<PlantillaFormType>({
    ...
    defaultValues: {
      ...
      largo_estandar_mm: initialData?.largo_estandar_mm
        ? Number(initialData.largo_estandar_mm)
        : 0,
      peso_teorico_kg: initialData?.peso_teorico_kg
        ? Number(initialData.peso_teorico_kg)
        : 0,
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Error al crear plantilla: " + error.message,
        variant: "destructive",
      });
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: PlantillaFormType) =>
      catApi.updatePlantilla(initialData.id_plantilla, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["catPlantillas"] });
      toast({
    ...
    onError: (error) => {
      toast({
        title: "Error al actualizar plantilla: " + error.message,
        variant: "destructive",
      });
    },
  });
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  placeholder="Ej. Felpa 80, Perfil Vertical Nave Top"
                  {...field}
                />
              </FormControl>
              <FormMessage />
