      ...
      {focused && (
        <div className="absolute z-50 top-full left-0 mt-1 w-80 bg-white border border-slate-200 rounded-lg shadow-lg p-3 text-xs">
          ...
          <div className="grid grid-cols-2 gap-1 mb-2">
            {FORMULA_VARIABLES.map((v) => (
              <div key={v.name} className="flex items-center gap-1">
                <code className="bg-slate-100 px-1 rounded text-blue-700">
                  {v.name}
                </code>
          ...
          <div className="grid grid-cols-2 gap-1">
            {FORMULA_FUNCTIONS.map((f) => (
              <div key={f.name} className="flex items-center gap-1">
                <code className="bg-slate-100 px-1 rounded text-purple-700">
                  {f.name}
                </code>
              </div>
            ))}
          ...
          <p className="text-slate-400 mt-2">
            Ejemplo: <code>(ancho/2)+4</code> → {previewVars.ancho / 2 + 4}
          </p>
        </div>
      )}
    </div>
