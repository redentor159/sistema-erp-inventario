...
export function RecipeMassAudit({
  onNavigate,
}: {
  ...
  const errors = useMemo(() => {
    ...
    data.forEach((r) => {
      ...
      if (isPerfil && !r.id_plantilla) {
        found.push({
          ...r,
          _errorType: "MISSING_PLANTILLA",
          _errorMsg: "Falta ID Plantilla",
        });
      }
      ...
      if (isAccesorio && r.id_sku_catalogo) {
        ...
        if (precio <= 0) {
          found.push({
            ...r,
            _errorType: "ZERO_PRICE",
            _errorMsg: "Costo = S/ 0.00",
          });
        }
      }
      ...
      if (r.formula_cantidad) {
        ...
        if (!val.valid) {
          found.push({
            ...r,
            _errorType: "FORMULA_ERROR",
            _errorMsg: `Form. Cantidad: ${val.error}`,
          });
        }
      }
      if (r.formula_perfil) {
        ...
        if (!val.valid) {
          found.push({
            ...r,
            _errorType: "FORMULA_ERROR",
            _errorMsg: `Form. Perfil: ${val.error}`,
          });
        }
      }
    });
  ...
  const sortedErrors = useMemo(() => {
    return [...errors].sort(
      (a, b) => a.modelo_nombre?.localeCompare(b.modelo_nombre) || 0,
    );
  }, [errors]);

  const totalAudited =
    new Set(
      data?.filter((r) => r.modelo_activo !== false).map((r) => r.id_modelo),
    ).size || 0;
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[90vw] w-full h-[85vh] flex flex-col p-4">
        <DialogHeader className="flex flex-row items-center justify-between pb-2 shrink-0 border-b">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              El sistema ha cargado {totalLines} registros de {totalAudited}{" "}
              modelos activos.
            </p>
          </div>
          <Button
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-green-600 bg-green-50/50 absolute inset-0">
              ...
              <p className="text-sm text-green-700 mt-2">
                No se encontraron errores de configuración ni costos nulos.
              </p>
            </div>
          ) : (
              ...
              <Table>
                ...
                <TableBody>
                  {sortedErrors.map((err, i) => {
                    ...
                    if (err._errorType === "FORMULA_ERROR")
                      bgClass = "bg-red-50 text-red-700 border-red-200";
                    if (err._errorType === "ZERO_PRICE")
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <div
                            className={`text-xs px-2.5 py-1 rounded-md border font-medium ${bgClass}`}
                          >
                            {err._errorMsg}
                          </div>
                        </TableCell>
        ...
        <div className="mt-3 text-xs flex items-center justify-between shrink-0 bg-slate-50 border rounded-md px-4 py-2">
          <span className="text-slate-600 font-medium tracking-wide uppercase">
            Diagnóstico FInal
          </span>
          {isLoading ? (
            ...
            <span className="font-bold text-red-600">
              {sortedErrors.length} Riesgo(s) Detectado(s) en{" "}
              {new Set(sortedErrors.map((e) => e.id_modelo)).size} modelo(s)
            </span>
          ) : (
