...
export const kanbanApi = {
    ...
    // Create a new order
    createOrder: async (order: Omit<KanbanOrder, 'created_at' | 'updated_at'>) => {
        const { data, error } = await supabase
            .from('trx_kanban_orders')
            .insert(order)
            .select()
    ...
    moveCard: async (id: string, columnId: string, newIndex: number) => {
        ...
        const latestMove = currentMovements.length > 0 ? currentMovements[currentMovements.length - 1] : null

        const newMovements = [...currentMovements]

        if (latestMove && !latestMove.exitDate) {
            // Close previous
            newMovements[newMovements.length - 1] = { ...latestMove, exitDate: new Date().toISOString() }
        }
    ...
    archiveOrder: async (id: string, status: 'Finalizado' | 'Eliminado' | 'Archivado', deletedFrom: string) => {
    ...
    // Update Config
    updateConfig: async (settings: { company_name?: string, wip_limits?: any }) => {
        const { data, error } = await supabase
            .from('mst_kanban_config')
            .update(settings)
            .eq('config_key', 'main_config')
    ...
    generateDemoData: async () => {
        ...
        const stages = ['column-pedidos-confirmados', 'column-en-corte', 'column-en-ensamblaje', 'column-listo-para-instalar']
        ...
        for (let i = 0; i < years * itemsPerYear; i++) {
            ...
            for (const stage of stages) {
                ...
                const daysInStage = Math.floor(Math.random() * (max - min + 1)) + min + Math.random() // Add decimal variance
            ...
            demoHistory.push({
                ...
                rework_history: hasRework ? [{ date: completion.toISOString().split('T')[0], from: COLUMN_TITLES_MAP['column-en-ensamblaje'], to: COLUMN_TITLES_MAP['column-en-corte'] }] : [],
