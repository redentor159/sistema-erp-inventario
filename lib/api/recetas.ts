...
export const recetasApi = {
  ...
  createModelo: async (
    modelo: Omit<RecetaModelo, "created_at" | "mst_series_equivalencias">,
  ): Promise<RecetaModelo> => {
    const { data, error } = await supabase
      .from("mst_recetas_modelos")
      .insert(modelo)
      .select()
  ...
  updateModelo: async (
    id_modelo: string,
    updates: Partial<RecetaModelo>,
  ): Promise<RecetaModelo> => {
    const {
      mst_series_equivalencias: _,
      created_at: __,
      ...clean
    } = updates as any;
    const { data, error } = await supabase
  ...
  updateRecetaLinea: async (
    id_receta: string,
    updates: Partial<RecetaLineaForm>,
  ): Promise<RecetaLinea> => {
  ...
  getAllRecetasConCatalogInfo: async (): Promise<any[]> => {
    ...
    const variantMap = new Map(
      variantes?.map((v) => [v.id_sku, v.costo_mercado_unit]),
    );
    ...
    return recetas.map((r) => ({
      ...
      precio_catalogo: r.id_sku_catalogo
        ? variantMap.get(r.id_sku_catalogo)
        : null,
    }));
  },
  ...
  getAccesoriosConPrecio: async (id_modelo: string): Promise<any[]> => {
    ...
    return recetas.map((r) => {
      const variante = variantes?.find(
        (v) => v.id_plantilla === r.id_plantilla,
      );
      return {
        ...r,
  ...
  buscarProductosCatalogo: async (
    ...
    if (search) {
      query = query.or(
        `id_sku.ilike.%${search}%,nombre_completo.ilike.%${search}%`,
      );
    }

    query = query.order("id_sku", { ascending: true }).range(0, pageSize - 1);
