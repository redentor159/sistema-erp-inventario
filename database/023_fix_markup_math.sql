...
-- Fixes the math application of markup_aplicado in the despiece transaction
...
BEGIN
    ...
    -- Manejo del Flete Dinamico
    IF v_linea.opciones_seleccionadas ? 'factor_flete' THEN
        IF (v_linea.opciones_seleccionadas->>'factor_flete') = 'Otro' THEN
            v_factor_flete := NULLIF((v_linea.opciones_seleccionadas->>'factor_flete_otro'), '')::NUMERIC;
        ELSE
            v_factor_flete := NULLIF((v_linea.opciones_seleccionadas->>'factor_flete'), '')::NUMERIC;
        END IF;
    END IF;
    ...
    LOOP
        ...
        IF v_cantidad_calculada > 0 THEN
            ...
            INSERT INTO trx_desglose_materiales (
                ...
                precio_venta_item,      -- Added for UI fallback
                medida_corte_mm,
                codigo_base,
                angulo_corte,
                detalle_acabado
    ...
    IF v_linea.tipo_vidrio IS NOT NULL AND v_linea.tipo_vidrio <> '' THEN
        ...
        IF v_costo_vidrio IS NOT NULL THEN
            ...
            INSERT INTO trx_desglose_materiales (
                ...
                v_area_total * COALESCE(v_costo_vidrio, 0),
    ...
    IF COALESCE(v_factor_flete, 0) > 0 THEN
             ...
             BEGIN
                 SELECT costo_mercado_unit INTO v_precio_unit_flete 
                 FROM cat_productos_variantes WHERE id_sku = v_sku_flete;

                 IF v_precio_unit_flete IS NULL THEN 
                     v_precio_unit_flete := 0.60;
                 END IF;
                 
                 v_cantidad_flete := v_area_total * v_factor_flete;
                 v_costo_flete := v_cantidad_flete * v_precio_unit_flete;
                 ...
                 INSERT INTO trx_desglose_materiales (
                    ...
                    p_id_linea_cot,
                    'Servicio',
                    CASE WHEN v_es_templado THEN 'Costo Adicional: Embalaje y Flete del cristal' ELSE 'Costo Adicional: Embalaje y Flete' END,
                    v_sku_flete,
                    v_cantidad_flete,
                    v_costo_flete,
                    v_costo_flete
    ...
    SET 
        ...
        -- Subtotal Linea = Costo Unitario * (1 + Markup) * Cantidad
        -- FIX: Antes era (v_costo * COALESCE(markup)), lo que resultaba en un descuento masivo
        subtotal_linea = (v_costo_total_item * (1 + COALESCE(v_cotizacion.markup_aplicado, 0.35))) * v_linea.cantidad
    WHERE id_linea_cot = p_id_linea_cot;
