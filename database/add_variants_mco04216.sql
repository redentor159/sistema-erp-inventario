-- =================================================================================================
-- SEED: AGREGAR NUEVA PLANTILLA MCO04216 Y SUS VARIANTES
-- Fecha: 2026-03-19
--
-- Plantilla: Traslape rectangular reforzado (MCO04216)
...
VALUES ('MCO04216', 'Traslape rectangular reforzado', 'PERF', 'SYS_28', 6000)
...
-- Usamos una tabla temporal o valores directos para el CROSS JOIN
WITH marcas AS (
...
SELECT 
    'AL-MCO04216-' || a.id_finish || '-' || m.id_brand,
    'MCO04216',
    m.id_brand,
    'AL', -- Material Aluminio (AL)
    a.id_finish,
    'Traslape rectangular reforzado ' || a.name_finish || ' ' || m.name_brand,
    'UND',
    'PERFILES',
    180.00, -- Precio estimado referencial
    'PEN'
FROM marcas m
...
-- 3. Verificación
-- SELECT COUNT(*) FROM cat_productos_variantes WHERE id_sku LIKE 'AL-MCO04216%';
