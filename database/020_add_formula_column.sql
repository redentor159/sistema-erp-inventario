-- 020_add_formula_column.sql
-- Add 'detalle_formula' column to trx_desglose_materiales to store the calculation formula
-- e.g. "Ancho - 50mm"

ALTER TABLE trx_desglose_materiales
ADD COLUMN IF NOT EXISTS detalle_formula TEXT;

-- Update the view to include this new column
...
SELECT 
    ...
    -- ID LINEA (CRITICO PARA FILTRADO)
    m.id_linea_cot,
    ...
    m.nombre_componente,
    m.sku_real,
    -- p.nombre_completo as descripcion_sku, -- Removed to simplify join if p is null
    COALESCE(p.nombre_completo, m.nombre_componente) as descripcion_sku,
    m.detalle_acabado,
    m.medida_corte_mm,
    m.angulo_corte,
    
    m.detalle_formula, -- NEW COLUMN
